...
@if(collect(request()->query())->except('page')->isNotEmpty())
    <div class="mb-3">
        ...
        <div class="d-flex flex-wrap gap-2">
            @if(request('floor_id'))<span class="badge bg-info text-dark">Floor: {{ $floors->find(request('floor_id'))->name ?? $floors->find(request('floor_id'))->floor_name }}</span>@endif
            @if(request('department_id'))<span class="badge bg-info text-dark">Department: {{ $departments->find(request('department_id'))->department_name }}</span>@endif
            @if(request('category_id'))<span class="badge bg-info text-dark">Category: {{ $categories->find(request('category_id'))->name }}</span>@endif
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function fetchDepartments(floorId, selectedDepartmentId = null) {
        // If no floor is selected, just show "All Departments"
        if (!floorId) {
    ...
    // On page load, if a floor is already selected, fetch its departments and select the correct one if it exists.
    if (initialFloorId) {
